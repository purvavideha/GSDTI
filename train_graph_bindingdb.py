...
def train_drug_graph(train_dataset, test_dataset, valid_dataset, model,  criterion2, sim_loss_fn, args):
    ...
    if args.load_sim:
        train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, sampler=train_sampler,
                                  collate_fn=lambda batch: custom_collate_fn(batch, bindingdb_tanimoto_matrix, bindingdb_tm_score_matrix))
        test_loader = DataLoader(dataset=test_dataset, batch_size=args.batch_size, sampler=test_sampler,
                                 collate_fn=lambda batch: custom_collate_fn(batch, davis_tanimoto_matrix, davis_tm_score_matrix))
        val_loader = DataLoader(dataset=valid_dataset, batch_size=args.batch_size, sampler=val_sampler,
                                collate_fn=lambda batch: custom_collate_fn(batch, davis_tanimoto_matrix, davis_tm_score_matrix))
        t_f = train_mlp_sim
    ...
    if args.use_wandb:
        ...
        wandb.init(
            ...
            name='bindingdb_dataset_davis2_with_sim',
            notes='1'
        )
    ...
    for epoch in range(args.epochs):
        print(f"Epoch {epoch}")
        train_metrics = t_f(model_engine, train_loader, optimizer, criterion2, device,sim_loss_fn,deepspeed=True)
        val_metrics = e_f(model_engine, val_loader, criterion2, device,sim_loss_fn)
    ...
    val_metrics = e_f(model_engine, val_loader, criterion2, device,sim_loss_fn,log=True)
    # Save the best model
...
if __name__ == "__main__":
    ...
    data_df = bindingdb_data_df
    data_df['Label'] = data_df['Label'].astype(int)
    train_df, test_df = train_test_split(data_df, test_size=0.2, stratify=data_df['Label'], random_state=41)
    valid_df = test_df
    num_negative = train_df['Label'].value_counts()[1] * 2
    negative_samples = train_df[train_df['Label'] == 0].sample(num_negative, random_state=42)
    positive_samples = train_df[train_df['Label'] == 1]
    sampled_train_df = pd.concat([positive_samples, negative_samples])
    
    
    test_dataset = GraphDataset_withsim(test_df, bindingdb_drug_df, bindingdb_target_df, bindingdb_drug_dict, bindingdb_graph_folderpath)
    valid_dataset = GraphDataset_withsim(test_df, bindingdb_drug_df, bindingdb_target_df, bindingdb_drug_dict, bindingdb_graph_folderpath)
    train_dataset = GraphDataset_withsim(sampled_train_df, bindingdb_drug_df, bindingdb_target_df, bindingdb_drug_dict, bindingdb_graph_folderpath)
